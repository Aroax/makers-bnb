...
class Request
  attr_reader :customer_id, :space_id, :space_name, :space_owner_id, :date_in, :date_out, :hero_image, :request_status

  def self.dashboard(customer_id: )
    result = DatabaseConnection.query(sql: "SELECT customer.id AS customer_id,booking.space_id AS space_id,space.name AS space_name,space.customer_id AS space_owner_id,space.description AS space_description,booking.date_in AS date_in,booking.date_out AS date_out,space.hero_image AS hero_image,booking.request AS request FROM booking JOIN customer ON booking.customer_id=customer.id JOIN space ON booking.space_id=space.id WHERE customer.id=$1;", params: [customer_id])
    p result
    i = 0
    # length = result.count
  ...
  def initialize(customer_id:,space_id:,space_name:,date_in:,date_out:,hero_image:,request_status:, space_owner_id:)
